...
buildLocalUrl: function(page) {	
    ...
    if (navigator.device.platform == "Android") {
        result=SFHybridApp.buildAppUrl("file:///android_asset/www", page);
    }
    else {
        result=page; 
    }
    SFHybridApp.logToConsole('>>>buildLocalUrl:'+page+'=>'+result);
    return result;
},
...
buildAppUrl: function(server, page) {
    ...
    SFHybridApp.logToConsole('>>>buildAppUrl:'+trimmedServer+' + '+trimmedPage);
...
buildFrontDoorUrl: function(instanceServer, startPage, accessToken) {
    ...
    SFHybridApp.logToConsole('>>>buildFrontDoorUrl:'+fullUrl);
    return fullUrl;
},
...
RemoteAppStartData: function(appStartUrl, isAbsoluteUrl, shouldAuthenticate) {
    ...
    SFHybridApp.logToConsole('>>>RemoteAppStartData:'+JSON.stringify(this));
},
